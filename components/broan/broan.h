...
template<typename T>
concept BroanFieldTypes = std::is_same_v<T, float> || std::is_same_v<T, uint8_t> || std::is_same_v<T, uint32_t>;
...
struct BroanField_t {
  ...
  void markDirty() { m_unLastUpdate = millis() - m_unPollRate; }
};
...
class BroanComponent : public Component, public uart::UARTDevice {
 ...
 private:
  ...
  void handleUnknownField(uint32_t nOpcodeHigh, uint32_t nOpcodeLow, uint8_t len, uint32_t i,
